...
function Chart({ all, view, setView, tf, symbol, mas, height }) {
  ...
  geo.current = { step, usableW, priceH, lo, hi, autoLo: aLo, autoHi: aHi, maxOff, minOff };
  ...
  return (
    <div ref={wrapRef} style={{ position: "relative", width: "100%", touchAction: "none", overscrollBehavior: "contain" }}>
      ...
      <svg ref={svgRef} viewBox={`0 0 ${w} ${H}`} width="100%" height={H} preserveAspectRatio="none" style={{ display: "block", cursor: drag.current ? (drag.current.mode === "scaleY" ? "ns-resize" : drag.current.mode === "scaleX" ? "ew-resize" : "grabbing") : cursor }}
        onPointerDown={onDown} onPointerMove={onMove} onPointerUp={onUp} onPointerLeave={(e) => { onUp(e); setHoverGi(null); }} onDoubleClick={onDouble}>
...
// ── Einzel-Chart-Kachel ────────────────────────────────────────────
function ChartPane({ cfg, cryptoSymbols, compact, initial, demo }) {
  ...
  useEffect(() => { setView((v) => ({ ...v, offset: 0, pCenter: null, pSpan: null })); setAnalysis(null); }, [symbol, market, tf]);
  ...
  return (
    <div style={{ background: C.bg, color: C.text, fontFamily: SANS, padding: 16, borderRadius: 14, maxWidth: layout === 1 ? 920 : 1340, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap", marginBottom: 14 }}>
        ...
        <button className="cs-lay" data-on={demo ? 1 : 0} onClick={() => setDemo((d) => !d)} title="Demo-Daten zum Testen der Gesten (ohne Netzwerk)">Demo</button>
      ...
      <div className="cs-grid" data-n={layout}>
        {PANE_DEFAULTS.slice(0, layout).map((d, i) => <ChartPane key={i} cfg={cfg} cryptoSymbols={cryptoSymbols} compact={layout > 1} initial={d} demo={demo} />)}
      </div>
