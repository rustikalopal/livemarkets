  ...
  return (
    <div className="cs-rk">
      ...
      <div className="cs-rk-body">
        ...
        <div style={{ fontSize: 9.5, letterSpacing: 1.2, textTransform: "uppercase", color: C.faint, fontWeight: 700, marginTop: 2 }} title="Entry/Stop/TP einzeln am Chart ziehen · Alt+Ziehen am Entry verschiebt die ganze Position">Levels · am Chart ziehbar <span style={{ textTransform: "none", letterSpacing: 0, fontWeight: 400 }}>· Alt = ganze Position</span></div>
        ...
        <div style={{ display: "flex", gap: 7 }}>
          <label style={{ flex: 1 }}>{lbl("Stop-Loss", C.down, "Kurs, bei dem die Position mit Verlust geschlossen wird. Begrenzt das Risiko; der Abstand zum Entry steuert die Positionsgröße.")}<NumInput className="cs-rk-inp" value={trade.stop} onCommit={(v) => setTrade((t) => t ? { ...t, stop: v } : t)} /></label>
          <label style={{ flex: 1 }}>{lbl("Take-Profit", C.up, "Zielkurs für den Ausstieg mit Gewinn. Abstand zum Entry bestimmt zusammen mit dem Stop das Chance-Risiko-Verhältnis.")}<NumInput className="cs-rk-inp" value={trade.tp} onCommit={(v) => setTrade((t) => t ? { ...t, tp: v } : t)} /></label>
        </div>
        ...
        <div className="cs-rk-grid" style={{ gridTemplateColumns: "repeat(auto-fit,minmax(104px,1fr))" }}>
...
export default function ChartStudio() {
  ...
  const css = `
    ...
    .cs-rk-tiles{display:grid;grid-template-columns:repeat(auto-fill,minmax(290px,1fr));gap:14px;margin-top:14px}
    ...
    .cs-rk-body{display:flex;flex-direction:column;gap:9px;padding:12px}
