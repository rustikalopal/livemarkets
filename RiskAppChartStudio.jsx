...
// ── Einzel-Chart-Kachel ────────────────────────────────────────────
function ChartPane({ cfg, cryptoSymbols, compact, initial, index, register }) {
  ...
  return (
    <div style={{ background: C.bg, color: C.text, fontFamily: SANS, padding: 16, borderRadius: 14, maxWidth: layout === 1 ? 920 : 1340, margin: "0 auto" }}>
      ...
      <div className="cs-grid" data-n={layout}>
        {PANE_DEFAULTS.slice(0, layout).map((d, i) => <ChartPane key={i} index={i} register={register} cfg={cfg} cryptoSymbols={cryptoSymbols} compact={layout > 1} initial={d} />)}
      </div>
