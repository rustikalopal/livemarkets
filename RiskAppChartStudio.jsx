...
// ── Chart ──────────────────────────────────────────────────────────
// Rubber-Band-Daempfung an den Raendern (iOS-artig)
function rubber(excess) { const M = 12; return M * (1 - 1 / (1 + excess / M)); }
function bandClamp(raw, max) { if (raw < 0) return -rubber(-raw); if (raw > max) return max + rubber(raw - max); return raw; }
...
function Chart({ all, view, setView, tf, symbol, mas, height }) {
  ...
  const gesture = useRef({ axis: null, t: 0 }), panRaw = useRef(0), settle = useRef(0), raf = useRef(0), viewRef = useRef(view);
  ...
  useEffect(() => () => { if (raf.current) cancelAnimationFrame(raf.current); clearTimeout(settle.current); }, []);

  const H = height, padT = 12, padB = 22, padR = 60, padL = 6, volH = height > 360 ? 60 : 42, gap = 8;
  const len = all.length, cnt = Math.max(20, Math.round(view.count)), off = Math.round(view.offset);
  const end = len - off, start = Math.max(0, end - cnt);
  ...
  const layout = useMemo(() => {
    ...
    const rightGap = Math.min(70, plotW * 0.08), usableW = plotW - rightGap;
    const n = vis.length, step = usableW / n, cx = (i) => padL + step * (i + 0.5), bw = Math.max(1, Math.min(11, step * 0.66));
    const volMax = Math.max(...vis.map((c) => c.v)) || 1, volTop = padT + priceH + gap, vy = (vv) => volTop + volH - (vv / volMax) * volH;
    return { plotW, usableW, priceH, lo, hi, y, n, step, cx, bw, volTop, vy };
  ...
  const cancelAnim = () => { if (raf.current) { cancelAnimationFrame(raf.current); raf.current = 0; } };
  const animateOffset = (to, dur) => {
    cancelAnim();
    const from = viewRef.current.offset, t0 = performance.now(); dur = dur || 340;
    const tick = (now) => {
      const k = Math.min(1, (now - t0) / dur), e = 1 - Math.pow(1 - k, 3);
      setView((v) => ({ ...v, offset: from + (to - from) * e }));
      if (k < 1) raf.current = requestAnimationFrame(tick); else raf.current = 0;
    };
  ...
  const scheduleSettle = () => {
    ...
    settle.current = setTimeout(() => {
      const v = viewRef.current, max = Math.max(0, len - Math.round(v.count));
      const c = Math.max(0, Math.min(max, v.offset));
      if (Math.abs(c - v.offset) > 0.02) { panRaw.current = c; animateOffset(c, 420); }
    }, 120);
  };
  ...
  useEffect(() => {
    ...
    const onWheel = (e) => {
      ...
      if (now - gesture.current.t > 150) { gesture.current.axis = null; panRaw.current = viewRef.current.offset; } // neue Geste
      gesture.current.t = now;
      if (e.ctrlKey) gesture.current.axis = "y";                       // Pinch -> Zoom
      else if (!gesture.current.axis) gesture.current.axis = Math.abs(e.deltaX) > Math.abs(e.deltaY) ? "x" : "y";
      if (gesture.current.axis === "x") {
        cancelAnim();
        const max = Math.max(0, len - Math.round(viewRef.current.count));
        panRaw.current -= e.deltaX / g.step;                          // fraktioniert, traegheitsfaehig
        setView((v) => ({ ...v, offset: bandClamp(panRaw.current, max) }));
        scheduleSettle();
      } else {
        cancelAnim();
        const x = vbX(e.clientX);
        const frac = x == null ? 1 : Math.max(0, Math.min(1, (x - padL) / g.usableW));
        const zf = Math.exp(e.deltaY * 0.0016);
        setView((v) => {
          const count = Math.min(LOAD, Math.max(20, v.count * zf));
          const idxUnder = (len - v.offset - v.count) + frac * v.count;
          const offset = Math.max(0, Math.min(len - Math.round(count), len - (idxUnder - frac * count) - count));
          panRaw.current = offset;
          return { count, offset };
        });
      }
    };
  ...
  const onDown = (e) => { cancelAnim(); drag.current = { x: vbX(e.clientX), offset: viewRef.current.offset }; panRaw.current = viewRef.current.offset; e.currentTarget.setPointerCapture?.(e.pointerId); };
  const onMove = (e) => {
    const g = geo.current; if (!g) return;
    const x = vbX(e.clientX); if (x == null) return;
    if (drag.current && drag.current.x != null) {
      const max = Math.max(0, len - Math.round(viewRef.current.count));
      panRaw.current = drag.current.offset + (x - drag.current.x) / g.step; // greifen: ziehen rechts = aelter
      setView((v) => ({ ...v, offset: bandClamp(panRaw.current, max) }));
      return;
    }
    setHover(Math.max(0, Math.min(vis.length - 1, Math.round((x - padL) / g.step - 0.5))));
  };
  const onUp = (e) => { if (drag.current) { drag.current = null; scheduleSettle(); } e.currentTarget.releasePointerCapture?.(e.pointerId); };
  ...
  const tlX = hover != null ? Math.max(padL, Math.min(w - padR - tlW, cx(hover) - tlW / 2)) : 0;
  ...
  return (
    <div ref={wrapRef} style={{ position: "relative", width: "100%", touchAction: "none", overscrollBehavior: "contain" }}>
      ...
      <svg ref={svgRef} viewBox={`0 0 ${w} ${H}`} width="100%" height={H} preserveAspectRatio="none" style={{ display: "block", cursor: drag.current ? "grabbing" : "crosshair" }}
        ...
        {hover != null && !drag.current && (<g pointerEvents="none">
          <line x1={cx(hover)} x2={cx(hover)} y1={padT} y2={H - padB} stroke={C.dim} strokeDasharray="2 3" strokeWidth={1} opacity={0.6} />
...
export default function ChartStudio() {
  ...
  return (
    <div style={{ background: C.bg, color: C.text, fontFamily: SANS, padding: 16, borderRadius: 14, maxWidth: layout === 1 ? 920 : 1340, margin: "0 auto" }}>
      ...
      <div style={{ marginTop: 14, fontSize: 10.5, color: C.faint, lineHeight: 1.6, fontFamily: MONO }}>Krypto: Binance (keyless). Aktien: Yahoo über CORS-Proxy. Trackpad: quer wischen = durch die Zeit, hoch/runter oder Pinch = zoomen (unter dem Cursor). Ziehen = greifen & schieben. Keine Anlageempfehlung.</div>
    </div>
  );
}
