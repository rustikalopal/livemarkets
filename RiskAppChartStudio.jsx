...
async function fetchQuote(market, symbol, cfg) {
  ...
  return { price: last.c, chg: dayPct(c) };
}
...
function Chart({ all, view, setView, tf, symbol, mas, height, hoverBus, index, measure, setMeasure, trade, setTrade, onActivate }) {
...
function ChartPane({ cfg, cryptoSymbols, compact, initial, index, register, hoverBus, demo, measure, setMeasure, fav, trade, setTrade, onActivate, riskDef }) {
  ...
  const ch = dayPct(all);   // % seit Open der ersten Kerze des aktuellen Tages
  ...
  return (
    <div style={{ background: C.panel, border: `1px solid ${C.line}`, borderRadius: 12, padding: 10 }}>
      ...
      <div ref={boxRef} style={{ background: C.panel2, border: `1px solid ${C.line}`, borderRadius: 9, padding: "4px 2px" }}>
        {err ? <div style={{ height: chartH, display: "grid", placeItems: "center", textAlign: "center", color: C.warn, fontFamily: MONO, fontSize: 12, padding: 14 }}>{err}</div> : <Chart all={all} view={view} setView={setView} tf={tf} symbol={symbol} mas={mas} height={chartH} hoverBus={hoverBus} index={index} measure={measure} setMeasure={setMeasure} trade={trade} setTrade={setTrade} onActivate={onActivate} />}
      </div>
    </div>
  );
}
