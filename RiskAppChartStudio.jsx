...
async function fetchStockCandles(symbol, tf, proxyOverride) {
  ...
  for (const px of chain) {
    try { const r = await fetch(px + encodeURIComponent(yurl)); if (!r.ok) continue; const got = await r.json(); if (got && got.chart) { d = got; break; } }
    catch (e) { /* nächsten Proxy probieren */ }
  }
  if (!d) throw new Error("Yahoo gerade nicht erreichbar — alle CORS-Proxys fehlgeschlagen (⚙︎ eigenen Proxy setzen)");
...
function Chart({ all, view, setView, tf, symbol, mas, height }) {
  ...
  useEffect(() => {
    ...
    const onWheel = (e) => {
      ...
      const frac = x == null ? 1 : Math.max(0, Math.min(1, (x - padL) / g.plotW));
      // horizontaler Anteil -> Pan (sub-kerzen-genau akkumuliert)
      panAccum.current += e.deltaX / g.step;
      const panWhole = Math.trunc(panAccum.current);
      panAccum.current -= panWhole;
      // vertikaler Anteil -> sanfter Zoom, verankert unter dem Cursor
      const zf = Math.exp(e.deltaY * 0.0016);
      setView((v) => {
        const count = Math.min(LOAD, Math.max(20, Math.round(v.count * zf)));
        const curStart = len - v.offset - v.count;
        const idxUnder = curStart + frac * v.count;
        const newStart = Math.round(idxUnder - frac * count);
        let offset = len - newStart - count + panWhole;
        offset = Math.max(0, Math.min(len - count, offset));
        return { count, offset };
      });
    };
...
export default function ChartStudio() {
  ...
  return (
    <div style={{ background: C.bg, color: C.text, fontFamily: SANS, padding: 16, borderRadius: 14, maxWidth: layout === 1 ? 920 : 1340, margin: "0 auto" }}>
      ...
      <div style={{ marginTop: 14, fontSize: 10.5, color: C.faint, lineHeight: 1.6, fontFamily: MONO }}>Krypto: Binance (keyless). Aktien: Yahoo über CORS-Proxy. Trackpad: quer wischen = pannen, hoch/runter = zoomen (cursor-verankert). Analyse liest das jeweilige Chartbild. Keine Anlageempfehlung.</div>
    </div>
  );
}
