 ...
 * 1–4 Kachel-Charts (Krypto via Binance keyless, Aktien via Twelve Data).
...
const TD_IV = { "15m": "15min", "1h": "1h", "4h": "4h", "1d": "1day", "1w": "1week" };
function tdTime(s) { return new Date(s.includes(" ") ? s.replace(" ", "T") : s).getTime(); }
async function fetchStockCandles(symbol, tf, key) {
  if (!key) throw new Error("Twelve-Data-Key fehlt (⚙︎)");
  const url = `https://api.twelvedata.com/time_series?symbol=${encodeURIComponent(symbol)}&interval=${TD_IV[tf]}&outputsize=${LOAD}&apikey=${key}`;
  let d;
  try { const r = await fetch(url); d = await r.json(); }
  catch (e) { throw new Error("Netzwerk/CORS — Twelve Data evtl. nicht browser-fähig auf deinem Account"); }
  if (!d || d.status === "error" || !d.values) throw new Error((d && d.message) || "Aktiendaten nicht verfügbar");
  return d.values.map((v) => ({ t: tdTime(v.datetime), o: +v.open, h: +v.high, l: +v.low, c: +v.close, v: +(v.volume || 0) })).sort((a, b) => a.t - b.t);
}
function fetchCandles(market, symbol, tf, cfg) {
  return market === "stock" ? fetchStockCandles(symbol, tf, cfg.twelveKey) : fetchCryptoCandles(symbol, tf);
}
...
export default function ChartStudio() {
  ...
  const [cfg, setCfg] = useState({ anthropicKey: store.get("cs_anthropic") || "", model: store.get("cs_model") || DEFAULT_MODEL, twelveKey: store.get("cs_twelve") || "" });
  ...
  const saveCfg = () => { store.set("cs_anthropic", cfg.anthropicKey.trim()); store.set("cs_model", (cfg.model || DEFAULT_MODEL).trim()); store.set("cs_twelve", cfg.twelveKey.trim()); setCfg((c) => ({ anthropicKey: c.anthropicKey.trim(), model: (c.model || DEFAULT_MODEL).trim(), twelveKey: c.twelveKey.trim() })); setShowCfg(false); };
    ...
    <div style={{ background: C.bg, color: C.text, fontFamily: SANS, padding: 16, borderRadius: 14, maxWidth: layout === 1 ? 920 : 1340, margin: "0 auto" }}>
      ...
      {showCfg && (
        <div style={{ background: C.panel, border: `1px solid ${C.line}`, borderRadius: 10, padding: 14, marginBottom: 14, display: "grid", gap: 9, maxWidth: 560 }}>
          ...
          <input className="cs-inp" type="password" placeholder="Twelve-Data-Key — für Aktien (twelvedata.com, gratis)" value={cfg.twelveKey} onChange={(e) => setCfg({ ...cfg, twelveKey: e.target.value })} />
      ...
      <div style={{ marginTop: 14, fontSize: 10.5, color: C.faint, lineHeight: 1.6, fontFamily: MONO }}>Krypto: Binance (keyless). Aktien: Twelve Data (Key). Ziehen = Pan, Rad/±-Buttons = cursor-verankerter Zoom. Analyse liest das jeweilige Chartbild. Keine Anlageempfehlung.</div>
    </div>
  );
}
