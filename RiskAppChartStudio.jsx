...
// ── Screener ───────────────────────────────────────────────────────
async function klinesLite(symbol, interval, limit) {
  const d = await j(`https://api.binance.com/api/v3/klines?symbol=${symbol}&interval=${interval}&limit=${limit}`);
  return d.map((k) => ({ t: k[0], o: +k[1], h: +k[2], l: +k[3], c: +k[4], v: +k[5] }));
}
// Donchian-Breakout: Schluss der letzten Kerze über dem N-Perioden-Hoch (up) bzw. unter dem N-Tief (down) der VORHERIGEN N Kerzen.
function donchianSignal(c, n) {
  if (!c || c.length < n + 1) return null;
  const last = c[c.length - 1];
  let hh = -Infinity, ll = Infinity;
  for (let i = c.length - 1 - n; i < c.length - 1; i++) { if (c[i].h > hh) hh = c[i].h; if (c[i].l < ll) ll = c[i].l; }
  if (last.c > hh) return { dir: "up", level: hh, dist: (last.c / hh - 1) * 100 };
  if (last.c < ll) return { dir: "down", level: ll, dist: (last.c / ll - 1) * 100 };
  return null;
}
// Relatives Volumen: Volumen der letzten Kerze ÷ Median der vorherigen K Kerzen.
function rvolOf(c, k) {
  if (!c || c.length < k + 1) return null;
  const last = c[c.length - 1], vols = [];
  for (let i = c.length - 1 - k; i < c.length - 1; i++) if (c[i].v > 0) vols.push(c[i].v);
  if (!vols.length) return null;
  vols.sort((a, b) => a - b);
  const med = vols[Math.floor(vols.length / 2)];
  return med > 0 ? last.v / med : null;
}
async function mapLimit(items, limit, fn, onProgress, isAborted) {
...
const STABLE_BASES = new Set(["USDC", "FDUSD", "TUSD", "USDP", "DAI", "BUSD", "EUR", "GBP", "AEUR", "EURI", "USD1", "XUSD"]);
function screenableUSDT(sym, validSet) {
  if (!sym.endsWith("USDT")) return false;
  const base = sym.slice(0, -4);
  if (STABLE_BASES.has(base)) return false;
  if (/(UP|DOWN|BULL|BEAR)$/.test(base)) return false;            // gehebelte Token ausschließen
  return !validSet || validSet.has(sym);
}
async function runScreener({ tf, n, k, topN, validSet, onProgress, isAborted }) {
  const all = await j("https://api.binance.com/api/v3/ticker/24hr");   // ganzer Markt in EINEM Aufruf
  if (!Array.isArray(all)) throw new Error("unerwartete Marktdaten");
  const uni = all
    .filter((t) => screenableUSDT(t.symbol, validSet))
    .map((t) => ({ s: t.symbol, price: +t.lastPrice, chg: +t.priceChangePercent, qv: +t.quoteVolume }))
    .filter((t) => t.qv > 0 && isFinite(t.price) && t.price > 0)
    .sort((a, b) => b.qv - a.qv)
    .slice(0, topN);
  onProgress && onProgress(0, uni.length);
  const need = Math.max(n, k) + 2;
  const rows = await mapLimit(uni, 6, async (u) => {
    const c = await klinesLite(u.s, tf, Math.min(400, need + 5));
    const sig = donchianSignal(c, n);
    if (!sig) return null;
    return { ...u, dir: sig.dir, level: sig.level, dist: sig.dist, rvol: rvolOf(c, k) };
  }, onProgress, isAborted);
...
function Chart({ all, view, setView, tf, symbol, mas, height, hoverBus, index, measure, setMeasure, trade, setTrade, onActivate, showSess }) {
  ...
  const onMove = (e) => {
    ...
    setHoverY(pt.y);
    const overHandle = trade && pt.x >= padL && pt.x < w - padR && [trade.entry, trade.stop, trade.tp].some((pp) => Math.abs(y(pp) - pt.y) <= 6);
    setCursor(overHandle ? "ns-resize" : pt.x >= w - padR ? "ns-resize" : pt.y >= H - padB ? "ew-resize" : "crosshair");
...
// ── Einzel-Chart-Kachel ────────────────────────────────────────────
function ChartPane({ cfg, cryptoSymbols, compact, initial, index, register, hoverBus, demo, measure, setMeasure, fav, trade, setTrade, onActivate, riskDef }) {
  ...
  return (
    <div style={{ background: C.panel, border: `1px solid ${C.line}`, borderRadius: 12, padding: 10 }}>
      ...
      <div ref={boxRef} style={{ background: C.panel2, border: `1px solid ${C.line}`, borderRadius: 9, padding: "4px 2px" }}>
        {err ? <div style={{ height: chartH, display: "grid", placeItems: "center", textAlign: "center", color: C.warn, fontFamily: MONO, fontSize: 12, padding: 14 }}>{err}</div> : <Chart all={all} view={view} setView={setView} tf={tf} symbol={symbol} mas={mas} height={chartH} hoverBus={hoverBus} index={index} measure={measure} setMeasure={setMeasure} trade={trade} setTrade={setTrade} onActivate={onActivate} showSess={showSess} />}
      </div>
    </div>
  );
}
...
function TradeWindow({ trade, setTrade, setDefaults, onClose }) {
...
export default function ChartStudio() {
  ...
  const [scanN, setScanN] = useState(20);
  const [scanTopN, setScanTopN] = useState(120);
  const [scanRows, setScanRows] = useState([]);
  const [scanBusy, setScanBusy] = useState(false);
  const [scanProg, setScanProg] = useState({ done: 0, total: 0 });
  const [scanMsg, setScanMsg] = useState("");
  const [scanAt, setScanAt] = useState(null);
  const [scanDir, setScanDir] = useState("all");
  const [scanMinRvol, setScanMinRvol] = useState(0);
  const [scanSort, setScanSort] = useState("rvol");
  ...
  const cryptoSet = useMemo(() => new Set(cryptoSymbols.map((s) => s.s)), [cryptoSymbols]);
  const doScan = async () => {
    if (scanBusy) { scanAbort.current = true; setScanMsg("breche ab …"); return; }   // Button dient auch als Abbruch
    scanAbort.current = false;
    setScanBusy(true); setScanRows([]); setScanProg({ done: 0, total: 0 }); setScanMsg("lade Marktdaten …");
    try {
      const rows = await runScreener({
        tf: scanTf, n: Math.max(2, scanN | 0), k: 20, topN: Math.max(10, Math.min(400, scanTopN | 0)),
        validSet: cryptoSet.size ? cryptoSet : null,
        onProgress: (done, total) => setScanProg({ done, total }),
        isAborted: () => scanAbort.current,
      });
      if (scanAbort.current) { setScanRows(rows); setScanMsg(`abgebrochen — ${rows.length} Treffer aus Teilscan.`); setScanAt(Date.now()); }
      else { setScanRows(rows); setScanAt(Date.now()); setScanMsg(rows.length ? "" : "keine Breakouts — andere Zeitebene oder größeres Universum probieren."); }
    } catch (e) { setScanMsg("Scan fehlgeschlagen: " + (e.message || "Fehler") + " · ggf. Sandbox/Netz prüfen"); }
    finally { setScanBusy(false); }
  };
  const scanView = scanRows
    .filter((r) => (scanDir === "all" || r.dir === scanDir) && (r.rvol || 0) >= scanMinRvol)
    .sort((a, b) => scanSort === "chg" ? Math.abs(b.chg) - Math.abs(a.chg) : (b.rvol || 0) - (a.rvol || 0));
  ...
  const buildConfig = () => ({ version: 1, savedAt: new Date().toISOString(), watchlist: wl, risk: riskDef, layout, showWl, config: { model: cfg.model, yahooProxy: cfg.yahooProxy } }); // bewusst OHNE anthropicKey
  const applyConfig = (f) => {
  ...
  return (
    <div style={{ background: C.bg, color: C.text, fontFamily: SANS, padding: 16, borderRadius: 14, maxWidth: (layout === 1 ? 920 : 1340) + 260, margin: "0 auto", userSelect: "none", WebkitUserSelect: "none" }}>
      ...
      <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap", marginBottom: 14 }}>
        ...
        <button className="cs-lay" data-on={showScreen ? 1 : 0} style={{ marginLeft: "auto" }} title="Screener: neue Trades finden" onClick={() => setShowScreen((s) => !s)}>🔎 Screener</button>
        <button className="cs-lay" data-on={showWl ? 1 : 0} title="Watchlist ein-/ausblenden" onClick={() => setShowWl((s) => !s)}>★ Liste</button>
      ...
      {showScreen && (
        <div style={{ background: C.panel, border: `1px solid ${C.line}`, borderRadius: 10, padding: 14, marginBottom: 14, display: "grid", gap: 11 }}>
          <div style={{ display: "flex", alignItems: "baseline", gap: 9, flexWrap: "wrap" }}>
            <span style={{ fontSize: 13, fontWeight: 700, color: C.text }}>Screener · Krypto</span>
            <span style={{ fontSize: 11, color: C.dim }}>Donchian-Breakout + relatives Volumen über das liquideste USDT-Universum.</span>
            {scanAt && <span style={{ marginLeft: "auto", fontSize: 10.5, color: C.faint, fontFamily: MONO }}>Stand {new Date(scanAt).toLocaleTimeString("de-DE")}</span>}
          </div>

          <div style={{ display: "flex", gap: 14, alignItems: "flex-end", flexWrap: "wrap" }}>
            <div>
              <div style={{ fontSize: 10, color: C.faint, marginBottom: 4, textTransform: "uppercase", letterSpacing: 0.5 }}>Zeitebene</div>
              <div style={{ display: "flex", gap: 5 }}>{["1h", "4h", "1d"].map((t) => <button key={t} className="cs-tf" data-on={scanTf === t ? 1 : 0} onClick={() => setScanTf(t)}>{t}</button>)}</div>
            </div>
            <label style={{ display: "block" }}><div style={{ fontSize: 10, color: C.faint, marginBottom: 4, textTransform: "uppercase", letterSpacing: 0.5 }} title="Donchian-Lookback: Bruch des Hochs/Tiefs der letzten N Kerzen">Donchian N</div><input className="cs-inp" style={{ width: 70 }} type="number" min="3" value={scanN} onChange={(e) => setScanN(+e.target.value || 20)} /></label>
            <label style={{ display: "block" }}><div style={{ fontSize: 10, color: C.faint, marginBottom: 4, textTransform: "uppercase", letterSpacing: 0.5 }} title="Wie viele Paare (nach 24h-Volumen sortiert) im Feinpass geprüft werden">Universum (Top)</div><input className="cs-inp" style={{ width: 80 }} type="number" min="10" max="400" step="10" value={scanTopN} onChange={(e) => setScanTopN(+e.target.value || 120)} /></label>
            <button className="cs-btn" onClick={doScan} style={scanBusy ? { background: C.down } : null}>{scanBusy ? "Abbrechen" : "Scan starten"}</button>
            {scanBusy && scanProg.total > 0 && <span style={{ fontSize: 11.5, color: C.dim, fontFamily: MONO }}>{scanProg.done}/{scanProg.total} geprüft …</span>}
            {scanMsg && !scanBusy && <span style={{ fontSize: 11.5, color: scanRows.length ? C.dim : C.warn, lineHeight: 1.4 }}>{scanMsg}</span>}
            {scanBusy && scanMsg && scanProg.total === 0 && <span style={{ fontSize: 11.5, color: C.dim }}>{scanMsg}</span>}
          </div>

          {scanRows.length > 0 && (<>
            <div style={{ display: "flex", gap: 14, alignItems: "center", flexWrap: "wrap", borderTop: `1px solid ${C.line}`, paddingTop: 10 }}>
              <div style={{ display: "flex", gap: 5, alignItems: "center" }}>
                <span style={{ fontSize: 10.5, color: C.faint }}>Richtung:</span>
                {[["all", "Alle"], ["up", "Ausbruch ↑"], ["down", "Ausbruch ↓"]].map(([k, l]) => <button key={k} className="cs-tf" data-on={scanDir === k ? 1 : 0} onClick={() => setScanDir(k)}>{l}</button>)}
              </div>
              <label style={{ display: "flex", gap: 6, alignItems: "center", fontSize: 10.5, color: C.faint }}>min RVOL <input className="cs-inp" style={{ width: 56 }} type="number" min="0" step="0.5" value={scanMinRvol} onChange={(e) => setScanMinRvol(+e.target.value || 0)} /></label>
              <div style={{ display: "flex", gap: 5, alignItems: "center" }}>
                <span style={{ fontSize: 10.5, color: C.faint }}>Sortierung:</span>
                {[["rvol", "RVOL"], ["chg", "24h %"]].map(([k, l]) => <button key={k} className="cs-tf" data-on={scanSort === k ? 1 : 0} onClick={() => setScanSort(k)}>{l}</button>)}
              </div>
              <span style={{ marginLeft: "auto", fontSize: 11, color: C.dim, fontFamily: MONO }}>{scanView.length} Treffer</span>
            </div>
            <div style={{ maxHeight: 360, overflowY: "auto", border: `1px solid ${C.line}`, borderRadius: 8 }}>
              <table className="cs-scr">
                <thead><tr><th className="l">#</th><th className="l">Symbol</th><th>Preis</th><th>24h %</th><th>RVOL</th><th className="l">Signal</th><th>Dist</th></tr></thead>
                <tbody>
                  {scanView.map((r, i) => (
                    <tr key={r.s} onClick={() => { onPick(r.s, "crypto"); }} title="In Kachel 1 laden">
                      <td className="l" style={{ color: C.faint }}>{i + 1}</td>
                      <td className="l" style={{ color: C.text, fontWeight: 700 }}>{r.s.replace(/USDT$/, "")}<span style={{ color: C.faint, fontWeight: 400 }}>/USDT</span></td>
                      <td>{fmtPrice(r.price)}</td>
                      <td style={{ color: r.chg >= 0 ? C.up : C.down }}>{fmtPct(r.chg)}</td>
                      <td style={{ color: r.rvol >= 2 ? C.accent : C.text, fontWeight: r.rvol >= 2 ? 700 : 400 }}>{r.rvol != null ? r.rvol.toFixed(2).replace(".", ",") + "×" : "—"}</td>
                      <td className="l" style={{ color: r.dir === "up" ? C.up : C.down }}>{r.dir === "up" ? `▲ ${scanN}-Hoch` : `▼ ${scanN}-Tief`}</td>
                      <td style={{ color: C.dim }}>{(r.dist >= 0 ? "+" : "") + r.dist.toFixed(2).replace(".", ",")}%</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            </div>
            <div style={{ fontSize: 10.5, color: C.faint, lineHeight: 1.45 }}>Klick lädt das Symbol in Kachel 1. Hinweis: Die jüngste Kerze ist noch offen — RVOL ist daher tagsüber untertrieben; auf <b style={{ color: C.dim }}>1h/4h</b> ist der Effekt kleiner. Kandidatenliste, kein Signal — bitte selbst prüfen.</div>
          </>)}
        </div>
      )}
