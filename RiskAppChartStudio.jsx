...
function Chart({ all, view, setView, tf, symbol, mas, height }) {
  const wrapRef = useRef(null), svgRef = useRef(null), drag = useRef(null), geo = useRef(null), panAccum = useRef(0), gesture = useRef({ axis: null, t: 0 });
  ...
  useEffect(() => {
    ...
    const onWheel = (e) => {
      ...
      if (now - gesture.current.t > 160) gesture.current.axis = null; // neue Geste -> Achse neu bestimmen
      ...
      if (gesture.current.axis === "x") {
        // horizontal -> durch die Zeit scrollen (Wischen rechts = zurueck/aelter)
        panAccum.current -= e.deltaX / g.step;
        const whole = Math.trunc(panAccum.current);
        if (whole) { panAccum.current -= whole; setView((v) => ({ ...v, offset: Math.max(0, Math.min(len - Math.round(v.count), Math.round(v.offset) + whole)) })); }
      } else {
        // vertikal/Pinch -> sanft zoomen, unter dem Cursor verankert
        ...
        setView((v) => {
          ...
          const offset = Math.max(0, Math.min(len - Math.round(count), Math.round(len - (idxUnder - frac * count) - count)));
          return { count, offset };
        });
      }
    };
  ...
  const onMove = (e) => {
    ...
    if (drag.current && drag.current.x != null) {
      ...
      setView((v) => ({ ...v, offset: Math.max(0, Math.min(len - Math.round(v.count), Math.round(drag.current.offset) + dc)) }));
      return;
    }
