...
function tzClock(epochMs, tz) {
  let f = _tzFmt.get(tz);
  if (!f) { f = new Intl.DateTimeFormat("en-US", { timeZone: tz, hour: "2-digit", minute: "2-digit", weekday: "short", hour12: false }); _tzFmt.set(tz, f); }
  let h = 0, m = 0, wd = "Mon";
  for (const p of f.formatToParts(new Date(epochMs))) { if (p.type === "hour") h = +p.value; else if (p.type === "minute") m = +p.value; else if (p.type === "weekday") wd = p.value; }
  if (h === 24) h = 0;
  return { min: h * 60 + m, dow: _DOW[wd] != null ? _DOW[wd] : 1 };
}
  ...
  const sessionBands = useMemo(() => {
    ...
    for (const S of SESSIONS) {
      ...
      for (let k = 0; k < idxs.length; k++) {
        ...
        const gap = run && (c.t - run.lastT) > barMs * 1.5;
        if (inSess && (!run || gap)) { if (run) out.push({ S, from: run.from, to: run.to }); run = { from: gi, to: gi, lastT: c.t }; }
        else if (inSess) { run.to = gi; run.lastT = c.t; }
        else if (run) { out.push({ S, from: run.from, to: run.to }); run = null; }
      }
      if (run) out.push({ S, from: run.from, to: run.to });
    }
    return out;
  }, [showSess, tf, all, vFrom, vTo, step]);
